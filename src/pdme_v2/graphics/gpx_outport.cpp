...
GpxOutport::GpxOutport(QGraphicsScene * scene, Port *data):_portData(data) 
{
...
void GpxOutport::paint(QPainter * painter, const QStyleOptionGraphicsItem *, QWidget *)
{
  ...
  //painter->rotate(-rotationAngle(_myDirection));
...
void GpxOutport::resizeTo(QPointF p,Qt::Corner c)
{
...
void GpxOutport::resizeTo(QPoint p)
{
  GpxBlock::resizeTo(p);
  _width=std::max(_height,_width);
  _height=std::max(_height,_width);
...
QVariant GpxOutport::itemChange(GraphicsItemChange change, const QVariant & value)
{
