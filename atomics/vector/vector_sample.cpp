#include "vector_sample.h"
void vector_sample::init(double t,...) {
...
for (int i=0;i<Nargs;i++){
 ...
 getScilabVector(fvars[i],&Ni[i],parsd[i]);
       ...
       D0 = new Coupling("CoupledSampler");
        ...
        for (int i=0;i<N;i++){ 
          sprintf(childi,"Sample%i",i);
        	D1[i] = new samplehold(childi);
...
double vector_sample::ta(double t) {
//This function returns a double.
return D0->ta(t);
}
void vector_sample::dint(double t) {
D0->dint(t);
}
void vector_sample::dext(Event x, double t) {
...
Event vector_sample::lambda(double t) {
