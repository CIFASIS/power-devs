#include "qss_integrator_vec.h"
void qss_integrator_vec::init(double t,...) {
...
for (int i=0;i<Nargs;i++){
 ...
 getScilabVector(fvars[i],&Ni[i],parsd[i]);
       ...
       D0 = new Coupling("CoupledQSSInt");
        ...
        for (int i=0;i<N;i++){ 
          sprintf(childi,"qss_int%i",i);
        	D1[i] = new qss_integrator(childi);
				for (int j=0;j<Nargs;j++){
            if (i<Ni[j]) par[j]=parsd[j][i];
					sprintf(pars[j],"%1.16g",par[j]);
				}	
         	D1[i]->init(t,fvar1,pars[0],pars[1],pars[2]);
      	}	
...
double qss_integrator_vec::ta(double t) {
//This function returns a double.
return D0->ta(t);
}
void qss_integrator_vec::dint(double t) {
D0->dint(t);
}
void qss_integrator_vec::dext(Event x, double t) {
...
Event qss_integrator_vec::lambda(double t) {
