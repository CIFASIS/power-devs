#include "qss_sum_vec.h"
void qss_sum_vec::init(double t,...) {
       ...
       D0 = new Coupling("CoupledWSum");
        ...
        for (int i=0;i<N;i++){ 
          sprintf(childi,"qss_sum%i",i);
        	D1[i] = new qss_wsum(childi);
				for (int j=0;j<Nargs;j++){
            if (i<Ni[j]) par[j]=parsd[j][i];
					sprintf(pars[j],"%1.16g",par[j]);
				}	
         	D1[i]->init(t,pars[0],pars[1],pars[2],pars[3],pars[4],pars[5],pars[6],pars[7],ni);
      	}	
...
double qss_sum_vec::ta(double t) {
//This function returns a double.
return D0->ta(t);
}
void qss_sum_vec::dint(double t) {
D0->dint(t);
}
void qss_sum_vec::dext(Event x, double t) {
...
Event qss_sum_vec::lambda(double t) {
