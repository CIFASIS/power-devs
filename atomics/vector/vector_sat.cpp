#include "vector_sat.h"
void vector_sat::init(double t,...) {
...
for (int i=0;i<Nargs;i++){
 ...
 getScilabVector(fvars[i],&Ni[i],parsd[i]);
       ...
       D0 = new Coupling("CoupledSat");
        ...
        for (int i=0;i<N;i++){ 
          sprintf(childi,"Saturation%i",i);
        	D1[i] = new qss_saturation(childi);
...
double vector_sat::ta(double t) {
//This function returns a double.
return D0->ta(t);
}
void vector_sat::dint(double t) {
D0->dint(t);
}
void vector_sat::dext(Event x, double t) {
...
Event vector_sat::lambda(double t) {
