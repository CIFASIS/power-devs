#include "vector_cross_det.h"
void vector_cross_det::init(double t,...) {
...
for (int i=0;i<Nargs;i++){
 ...
 getScilabVector(fvars[i],&Ni[i],parsd[i]);
       ...
       D0 = new Coupling("CoupledCrossd");
        ...
        for (int i=0;i<N;i++){ 
          sprintf(childi,"CrossDet%i",i);
        	D1[i] = new cross_detect(childi);
...
double vector_cross_det::ta(double t) {
//This function returns a double.
return D0->ta(t);
}
void vector_cross_det::dint(double t) {
D0->dint(t);
}
void vector_cross_det::dext(Event x, double t) {
...
Event vector_cross_det::lambda(double t) {
