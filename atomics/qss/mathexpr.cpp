...
if(s1[0]=='('&&SearchCorOpenbracket(s1,0)==s2-s1-1){
	s1++;s2--;ind=1;}
if(s1==s2){delete[]s;s=new char[1];return;}
...
int IsVar(const char*s,int n,int nvar,PRVar*ppvar)
{
  ...
  int i;
  for(i=0;i<nvar;i++)if(CompStr(s,n,(*(ppvar+i))->name))return strlen((*(ppvar+i))->name);
  return 0;
}
...
void IsolateVars(char*&s,int nvar,PRVar*ppvar)//Deletes the old string
{
  int i,j;
  i=0;
  for(i=0;s[i];i++){
    if(s[i]=='('){i=SearchCorOpenbracket(s,i);if(i==-1)return;continue;};
    if((j=IsVar(s,i,nvar,ppvar))||((CompStr(s,i,"pi")||CompStr(s,i,"PI")||CompStr(s,i,"Pi"))&&(j=2))){
      InsStr(s,i,'(');InsStr(s,i+j+1,')');i+=j+1;continue;};
    if(IsFunction(s,i)){i+=IsFunction(s,i)-1;if(!s[i])return;continue;};
  };
}
...
void IsolateNumbers(char*&s,int nvar,RVar**ppvar,int nfunc,RFunction**ppfunc)//Deletes the old string
{
  ...
  for(i=0;s[i];i++){
    ...
    if((t=IsVar(s,i,nvar,ppvar))){i+=t-1;continue;};
    if((t=IsFunction(s,i,nfunc,ppfunc))){i+=t-1;continue;};
    if(s[i]=='('){i=SearchCorOpenbracket(s,i);if(i==-1)return;continue;};
...
ROperation::ROperation(char*sp,int nvar,PRVar*ppvarp,int nfuncp,PRFunction*ppfuncp)
{
  ...
  if(EqStr(s,"pi")||EqStr(s,"PI")||EqStr(s,"Pi"))
	...
	if(nvar)IsolateVars(s,nvar,ppvarp);
	for(k=0;s[k];k++){
    if(s[k]=='('){k=SearchCorOpenbracket(s,k);if(k==-1)break;continue;};
    if(IsFunction(s,k,nfuncp,ppfuncp)){
      i=k+IsFunction(s,k,nfuncp,ppfuncp);while(s[i]==' ')i++;
      if(s[i]=='('){
