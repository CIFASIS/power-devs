#include "adquidervec.h"
void adquidervec::init(double t,...) {
...
//	%Type% is the parameter type
	...
	if(d<0.5) { 
		...
		tipomaq=pendubot_control;	//1 PenduBot control de corriente
	}
	...
	//printLog("\nCuantos:[%d,%d,%d], gainADC's:[%d,%d,%d], SatDer2:[%d,%d], SatDer3:[%d,%d], RefPos:%d\n",(int)Cuanto1,(int)Cuanto2,(int)Cuanto3,(int)gainADC1,(int)gainADC2,(int)gainADC3,(int)d2sup,(int)d2inf,(int)d3sup,(int)d3inf,(int)refpos);
...
if(tipomaq==levitador){ //si es el Levitador Magnetico
	...
	EscribirEPP(SetCuanto2);evA3[0];	//solicita cambio de cuanto
...
if(tipomaq==pendubot_control){
	ModoControl=1;
	tipomaq = pendubot;
	printLog("\n******** Pendubot en modo control de corriente **********\n");
}
if(tipomaq==pendubot){  //si es un pendubot
...
//printLog("cambio9\n");
	...
	refpos=ADC3; //Modificado Gustavo
	//if(dADC3[0]<0)dADC3[0]+=360.0; Cambio Gustavo
	...
	if(ModoControl)
	{
		EscribirEPP(ContCorrOn);
		EscribirEPP((unsigned char)((((ZERO_COR)>>4)&0xE0)|SetPWMAlta));	//genera comando y envia
		EscribirEPP((unsigned char)(((ZERO_COR)>>1)&0xFF));		//envia parte baja referencia de corriente
	}
...
double adquidervec::ta(double t) {
//This function returns a double.
	return sigma;
}
void adquidervec::dint(double t) {
//printLog("interna dADC3=%f ADC3=%d\n",dADC3[0],ADC3);
if((t-evT3[evI3ant])>TMAX && dDADC3[0]>1e-10){
		 cambio3==1;
		 dDADC3[0]=0;
		 sigma=0;
		 return;
	...
	if((cambio1 + cambio2 + cambio3) >=1)
		sigma=0;
	else 
		sigma=TMAX;
}
void adquidervec::dext(Event x, double t) {
...
//     'e' is the time elapsed since last transition
	...
	double  delta;
	if(ERROR==1)return;
//printLog("Externa dADC3=%f ADC3=%d\n",dADC3[0],ADC3);
	switch(x.port){ 
		case -1:	//Interrupcion del EPP
			if(desinc>0){	//debe resincronizar
				...
				if(tipomaq == pendubot) {
						...
						EscribirEPP(LeerAltaADC3);	//comando para leer el ADC2 entero
						short	ADC3temp;
						ADC3temp=LeerEPP()<<8;	//Lee parte alta
						ADC3temp+=LeerEPP();	//Lee parte baja
						delta=((double)(ADC3temp-ADC3))*360.0/4000.0;
						ADC3=ADC3temp;
						if(delta>180.0)delta-=360.0;
						else if(delta<-180.0)delta+=360.0;
						dADC3[0]+=delta;
						if (dADC3[0]>=360.0) dADC3[0]-=360.0;
						else if(dADC3[0]<0.0) dADC3[0]+=360.0;
						...
						evI3ant=evI3;
						if(evI3==(evN3-1)){
							delta=evA3[evI3]-evA3[0]; //printLog("ACA1 dADC3=%f delta=%f evA3[0]=%f\n",dADC3,delta,evA3[0]);
							if(delta>2000)delta-=4000;
							else if(delta<-2000)delta+=4000;
							dDADC3[0]=delta/(evT3[evI3]-evT3[0]);
							evI3=0;
						}else{
							delta=evA3[evI3]-evA3[evI3+1]; //printLog("ACA1\n");
							if(delta>2000)delta-=4000;
							else if(delta<-2000)delta+=4000;
		...
		case 3:		//Cambio en la salida PWM
			...
			{	
				if(ModoControl){
					s=(*d)*(4096.0*1.1)/5.0+ZERO_COR;
					if(s>4095)s=4095;
					else if(s<0) s=0;
					EscribirEPP((unsigned char)(((s>>4)&0xE0)|SetPWMAlta));	//genera comando y envia
					EscribirEPP((unsigned char)((s>>1)&0xFF));		//envia parte baja PWM
					//rtai_print_to_screen("0 %g Escribiendo %d. Valor de entrada %g\n",getRealSimulationTime(),s,*d);
				}else{
					s=(short)((*d)*512.0/30.0)+512;
					if(s>1023)s=1023;
					else if(s<0) s=0;

					EscribirEPP((unsigned char)((((s>>3)&0x60)|0x80)|SetPWMAlta));	//genera comando y envia
					EscribirEPP((unsigned char)(s&0xFF));			//envia parte baja PWM
					//rtai_print_to_screen("1 %g Escribiendo %d. Valor de entrada %g\n",getRealSimulationTime(),s,*d);
				}
			}else{
				*d=(*d)*gainPWM;
				if(*d>1300){
		...
		case 5:		//Evento en RESINC2
			...
			if(tipomaq==pendubot){
				...
				dADC2[0]+=delta*polrel;
				//printLog("\n 2 ADC2:%d, dADC2:%f\n",ADC2,dADC2[0]);
				//if (dADC2[0]>=360.0) dADC2[0]-=360.0;  CAmbio Gustavo
				//else if(dADC2[0]<0.0) dADC2[0]+=360.0; CAmbio Gustavo
...
Event adquidervec::lambda(double t) {
//This function returns an Event:
//     Event(%&Value%, %NroPort%)
//where:
//     %&Value% points to the variable which contains the value.
//     %NroPort% is the port number (from 0 to n-1)
//printLog("lambda dADC3=%f ADC3=%d\n",dADC3[0],ADC3);
	if(cambio1==1){
		cambio1=0;
		if(tipomaq == levitador)
			dADC1[0]=ADC1/781.5;
		else
			dADC1[0]=((double)ADC1-ZERO_COR)*5.0/(4096.0*1.1);
		dADC1[0]*=gainADC1;
	...
	if(cambio2==1){
		...
		if(evnum2>=evN2){
			if(dDADC2[0]>d2sup)dADC2[1]=d2sup;
			else if(dDADC2[0]<d2inf)dADC2[1]=d2inf;
			else if(isnan(dDADC2[0])==0)dADC2[1]=dDADC2[0];
...
//		if(dADC3[0]<0)dADC3[0]+=360.0; Cambio Gustavo
		...
		if(evnum3>=evN3){
			if(dDADC3[0]>d3sup)dADC3[1]=d3sup;
			else if(dDADC3[0]<d3inf)dADC3[1]=d3inf;
			else if(isnan(dDADC3[0])==0)dADC3[1]=dDADC3[0];
...
void adquidervec::exit() {
//Code executed at the end of the simulation.
	if(ERROR==1)return;
	ushort maxerr1,maxerr2, maxerr3;
	if(ModoControl)
		EscribirEPP(ContCorrOff);
