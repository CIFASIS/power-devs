...
void pid_controler::init(double t,...) {
...
Kp=getScilabVar(fvar );
fvar= va_arg(parameters,char*);
Ki=getScilabVar(fvar );
fvar= va_arg(parameters,char*);
Kd=getScilabVar(fvar );
...
double pid_controler::ta(double t) {
//This function return a double.
return sigma;
}
void pid_controler::dint(double t) {
double s;
switch(met) {
case 1: // QSS
      X=X+sigma*u;
...
//    if(fabs(q/dQ)<1){q=0;};  //Corrige error num�rico en torno a cero
    if (u!=0) {
        sigma=(q-X)/u+2*eps;
    } else {
        sigma=INF;
...
void pid_controler::dext(Event x, double t) {
...
if(x.port==0){
	...
	in_a[1]=Aux[1];
	in_a[2]=Aux[2];
//	in_b[0]=in_b[0]+in_b[1]*e+in_b[2]*e*e;
//	in_b[1]=in_b[1]+in_b[2]*e;
}else{
	in_b[0]=Aux[0];
	in_b[1]=Aux[1];
	in_b[2]=Aux[2];

//	in_a[0]=in_a[0]+in_a[1]*e+in_a[2]*e*e;
//	in_a[1]=in_a[1]+in_a[2]*e;
}
//rtai_print_to_screen("En t=%g en la externa llego por el puerto %d el valor %g y resulta in_a=%g e in _b=%g\n",t,x.port,Aux[0],in_a[0],in_b[0]);
double Aux_local[10];
Aux_local[0]=in_a[0]-in_b[0];
Aux_local[1]=in_a[1]-in_b[1];
Aux_local[2]=in_a[2]-in_b[2];
if(Kp!=0){
switch(met) {
case 1: //QSS
      X=X+e*u;
...
case 2: // QSS2
        // double a,b,c,s;
        X=X+u*e+mu/2*e*e;
        u=Aux_local[0];  //input value
        mu=Aux_local[1];  //input slope
            if (sigma!=0){             //1
                  q=q+mq*e;
                  a=mu/2;
                  b=u-mq;
...
case 3: // QSS3
        ...
        u=Aux_local[0];    //input value
        mu=Aux_local[1]; //input slope
        pu=Aux_local[2]; //input derivative
         if(sigma!=0) {
...
case 5:
    ...
    if (t==0) {// initialization
        if (band==1) { //we need to send the output
            sigma=0;
   double estder;     } else {  //the output was already sent and there was a change in u
...
//printLog("1En t=%g en la externa llego por el puerto %d el valor %g y resulta in_a=%g e in _b=%g\n",t,x.port,Aux[0],in_a[0],in_b[0]);
sigma=0;
}
Event pid_controler::lambda(double t) {
...
case 5:
   ...
   // if(fabs(y[0]/dQ)<1){y[0]=0;};  //Corrige error num�rico en torno a cero
    if(met==5){y[0]=(y[0]+X+sigma*u)/2;};
break;
};
//printLog("en T=%g in_a=%g in_b=%g y el error =%g \n",t,in_a[0], in_b[0],u);
if(mu==0){
	y[0]=Kp*(u + mu * sigma + pu * pow(sigma,2))+Ki*y[0]+Kd*estder;
}else{
	y[0]=Kp*(u + mu * sigma + pu * pow(sigma,2))+Ki*y[0]+Kd*(mu+2*pu*sigma);

}
return Event(&y,0);
}
void pid_controler::exit() {
